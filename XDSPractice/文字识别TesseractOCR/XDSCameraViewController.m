...
@property (strong, nonatomic)G8RecognitionOperation * recognitionOp;
...
- (void)viewDidLoad {
    ...
    [self performSelector:@selector(setupTesseract) withObject:nil afterDelay:0.1];//解决跳转卡顿问题
}
- (void)setupTesseract{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (!_canRecognize) {
        @autoreleasepool {
            ...
            //            UIImage * scanImage=[UIImage imageWithCGImage:CGImageCreateWithImageInRect([image CGImage], scanImageFrame) scale:1 orientation:UIImageOrientationRight];//此处有内存泄露，改为以下代码----2016.6.17许杜生修改
            CGImageRef imageRef = CGImageCreateWithImageInRect([image CGImage], scanImageFrame);
            UIImage * scanImage=[UIImage imageWithCGImage:imageRef scale:1 orientation:UIImageOrientationRight];
...
- (void)performImageRecognition:(UIImage *)image{
    _recognitionOp.tesseract.image = [image g8_blackAndWhite];
    if ([_recognitionOp.tesseract recognize]) {
        _canRecognize = NO;
        dispatch_sync(dispatch_get_main_queue(), ^{
            NSString * recognizedText = _recognitionOp.tesseract.recognizedText;
            if (recognizedText.length) {
                self.scanResultLabel.text = recognizedText;
            }
        });
        sleep(1);
        NSString * result = _recognitionOp.tesseract.recognizedText;
        NSMutableCharacterSet * characterSet = [NSMutableCharacterSet lowercaseLetterCharacterSet]; //小写字母
        NSCharacterSet *uppercaseSet = [NSCharacterSet uppercaseLetterCharacterSet];//大写字母
        NSCharacterSet * decimalDigitSet = [NSCharacterSet decimalDigitCharacterSet];
        NSString * otherCharacterSet = @"+-*/%|,_ !.?@#$<>~()[]{} ";
        NSCharacterSet * otherSet = [NSCharacterSet characterSetWithCharactersInString:otherCharacterSet];
        [characterSet formUnionWithCharacterSet:uppercaseSet];//大小写字母
        [characterSet formUnionWithCharacterSet:decimalDigitSet];//数字+字母
        [characterSet formUnionWithCharacterSet:otherSet];
        
        result = [[result componentsSeparatedByCharactersInSet:[characterSet invertedSet]] componentsJoinedByString:@""];
        NSLog(@"result = %@", result);
